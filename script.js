...
/////////////////////////////
//button de bloqueio de da
////////////////////////////
document.addEventListener("DOMContentLoaded", () => {
  const btn = document.getElementById("start-download");
  if (!btn) return;

  // Bloqueia visualmente e funcionalmente
  btn.disabled = true;
  btn.style.cursor = "not-allowed";
  btn.style.opacity = "0.6";
  btn.title = "Este botão está bloqueado";

  // Impede qualquer clique, mesmo por JS
  btn.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    console.log("Botão bloqueado — ação ignorada.");
  });
});
...
////////////////////////////////////////////////////
document.addEventListener("DOMContentLoaded", () => {
  const btn = document.getElementById("stop-download");
  if (!btn) return;
